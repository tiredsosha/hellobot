...
app = Flask(__name__)
slack_event_handler = SlackEventAdapter(os.environ['slack_singing'],
                                        '/slack/events', app)


# email function
def email_passcode(to, ebody):
    addr_from = os.environ['sender_email']
        ...
        try:
            if user_id != None and bot_id != user_id:
                if dep_value == None:
                    ...
                    client.chat_postMessage(
                        channel=f'{user_id}',
                        text="Дорогая, Полина Евгеньевна, проверьте свою почту"
                    )
                    email_passcode(
                        email,
                        'я вас очень люблю, но пароль вы не получите, досвидули\nбубубуб\nбипка'
                    )
